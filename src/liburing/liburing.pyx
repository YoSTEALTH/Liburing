...
cdef class io_uring:
    ...
    def __cinit__(self):
        ...
        self.ptr = <io_uring_t*>calloc(1, sizeof(io_uring_t))
        if self.ptr is NULL:
            memory_error(self)
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(flags={self.ptr.flags!r}, ring_fd={self.ptr.ring_fd!r}, features={self.ptr.features!r}, enter_ring_fd={self.ptr.enter_ring_fd!r}, int_flags={self.ptr.int_flags!r})'
...
cpdef void io_uring_queue_exit(io_uring ring):
    ...
    # note:
    #   - if `ring_fd` is not set and this function is called it will segfault
    #   - its important to create `ring` using `calloc`, so `ring_fd` is set to `0`
...
cpdef inline void io_uring_cq_advance(io_uring ring, unsigned int nr):
    io_uring_cq_advance_c(ring.ptr, nr)

cpdef inline void io_uring_cqe_seen(io_uring ring, io_uring_cqe nr):
...
cpdef inline void io_uring_sqe_set_data64(io_uring_sqe sqe, __u64 data):
    io_uring_sqe_set_data64_c(sqe.ptr, data)

cpdef inline __u64 io_uring_cqe_get_data64(io_uring_cqe cqe):
    return io_uring_cqe_get_data64_c(cqe.ptr)


cpdef inline void io_uring_sqe_set_flags(io_uring_sqe sqe, unsigned int flags):
    io_uring_sqe_set_flags_c(sqe.ptr, flags)




cpdef inline void io_uring_prep_nop(io_uring_sqe sqe):
    io_uring_prep_nop_c(sqe.ptr)

cpdef inline void io_uring_prep_cancel64(io_uring_sqe sqe, __u64 user_data, int flags):
    io_uring_prep_cancel64_c(sqe.ptr, user_data, flags)

cpdef inline void io_uring_prep_cancel(io_uring_sqe sqe, object user_data, int flags):
    Py_INCREF(user_data)
    io_uring_prep_cancel_c(sqe.ptr, <void*>user_data, flags)

cpdef inline void io_uring_prep_cancel_fd(io_uring_sqe sqe, int fd, unsigned int flags):
...
cpdef inline unsigned int io_uring_sq_ready(io_uring ring):
    return io_uring_sq_ready_c(ring.ptr)

cpdef inline unsigned int io_uring_sq_space_left(io_uring ring):
    return io_uring_sq_space_left_c(ring.ptr)

cpdef inline int io_uring_sqring_wait(io_uring ring):
    return io_uring_sqring_wait_c(ring.ptr)

cpdef inline unsigned int io_uring_cq_ready(io_uring ring):
    return io_uring_cq_ready_c(ring.ptr)

cpdef inline bool io_uring_cq_has_overflow(io_uring ring):
    return io_uring_cq_has_overflow_c(ring.ptr)

cpdef inline bool io_uring_cq_eventfd_enabled(io_uring ring):
    return io_uring_cq_eventfd_enabled_c(ring.ptr)

cpdef inline int io_uring_cq_eventfd_toggle(io_uring ring, bool enabled):
...
cpdef inline int io_uring_peek_cqe(io_uring ring, io_uring_cqe cqe_ptr):
    return io_uring_peek_cqe_c(ring.ptr, &cqe_ptr.ptr)

cpdef inline int io_uring_wait_cqe(io_uring ring, io_uring_cqe cqe_ptr):
    return io_uring_wait_cqe_c(ring.ptr, &cqe_ptr.ptr)

cpdef inline int io_uring_buf_ring_mask(__u32 ring_entries):
    return io_uring_buf_ring_mask_c(ring_entries)

cpdef inline void io_uring_buf_ring_init(io_uring_buf_ring br):
...
cpdef inline void io_uring_buf_ring_advance(io_uring_buf_ring br, int count):
