...
cpdef int io_uring_wait_cqes(io_uring ring,
    ...
    return trap_error(io_uring_wait_cqes_c(ring.ptr, &cqe_ptr.ptr, wait_nr, ts.ptr, sigmask.ptr))
...
cpdef int io_uring_submit_and_wait_timeout(io_uring ring,
    ...
    return trap_error(io_uring_submit_and_wait_timeout_c(ring.ptr, &cqe_ptr.ptr, wait_nr, ts.ptr,
                                                         sigmask.ptr))
...
cpdef int io_uring_enter(unsigned int fd,
                         ...
                         unsigned int flags,
                         sigset sig) nogil:
    return trap_error(io_uring_enter_c(fd, to_submit, min_complete, flags, sig.ptr))
...
cpdef int io_uring_enter2(unsigned int fd,
                          ...
                          sigset sig,
                          size_t sz) nogil:
    return trap_error(io_uring_enter2_c(fd, to_submit, min_complete, flags, sig.ptr, sz))
